...
#[test]
fn search_rejects_nonnumeric_since_with_parse_code() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    run(&bus, &["claim", "a", "--workspace", "."]);
    ...
    let denied = run_fail(
        &bus,
        &[
            "search", "needle", "--agent", "a", "--since", "soon", "--json",
        ],
    );
    let err: serde_json::Value = serde_json::from_slice(&denied.stderr).unwrap();
    assert_eq!(err["error"]["code"], "parse");
    assert!(
        err["error"]["message"]
            .as_str()
            .unwrap()
            .contains("duration must be numeric")
    );
}
...
#[test]
fn search_rejects_unknown_since_unit_with_parse_code() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    run(&bus, &["claim", "a", "--workspace", "."]);
    ...
    let denied = run_fail(
        &bus,
        &[
            "search", "needle", "--agent", "a", "--since", "5w", "--json",
        ],
    );
    let err: serde_json::Value = serde_json::from_slice(&denied.stderr).unwrap();
    assert_eq!(err["error"]["code"], "parse");
...
#[test]
fn doctor_flags_obligations_on_non_obligation_kinds() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    ...
    message["requires_ack"] = serde_json::json!(true);
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "non_obligation_kind_has_ask")
    );
}

#[test]
fn doctor_flags_awaited_non_participants() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["needs_response_from"] = serde_json::json!(["mallory"]);
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "awaited_not_participant")
    );
}
...
#[test]
fn doctor_flags_invalid_message_senders() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["from"] = serde_json::json!("@alice");
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_sender_id")
    );
}

#[test]
fn doctor_flags_invalid_message_conversation_ids() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["conversation_id"] = serde_json::json!("@c");
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_message_conversation_id")
    );
}

#[test]
fn doctor_flags_invalid_message_recipients() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["to"] = serde_json::json!(["@bob"]);
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_recipient_id")
    );
}

#[test]
fn doctor_flags_duplicate_message_recipients() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["to"] = serde_json::json!(["bob", "bob"]);
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "duplicate_recipient")
    );
}

#[test]
fn doctor_flags_invalid_message_mentions() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["mentions"] = serde_json::json!(["@bob"]);
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_mention_id")
    );
}

#[test]
fn doctor_flags_duplicate_message_mentions() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["mentions"] = serde_json::json!(["bob", "bob"]);
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "duplicate_mention")
    );
}

#[test]
fn doctor_flags_invalid_awaited_agents() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["needs_response_from"] = serde_json::json!(["@bob"]);
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_awaited_id")
    );
}

#[test]
fn doctor_flags_invalid_after_ids() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["after"] = serde_json::json!("@parent");
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_after_id")
    );
}
...
fn doctor_flags_oversized_summary_messages() {
    ...
    let path = bus.join(format!("conversations/c/messages/{message_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["body"] = serde_json::json!("x".repeat(2001));
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "summary_too_large")
    );
}
...
#[test]
fn doctor_flags_invalid_task_capabilities() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
    ...
    body["capability"]["blocks"][0]["sig"] = serde_json::json!("00");
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_task_capability")
    );
}

#[test]
fn doctor_flags_task_without_worker() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/messages/{task_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["needs_response_from"] = serde_json::json!([]);
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_task_worker_count")
    );
}

#[test]
fn doctor_flags_task_requires_ack() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "task_requires_ack")
    );
}

#[test]
fn doctor_flags_invalid_task_results() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_task_result")
    );
}
...
fn doctor_flags_task_worker_not_addressed() {
    ...
    let path = bus.join(format!("conversations/c/messages/{task_id}.json"));
    let mut message: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    message["to"] = serde_json::json!(["alice"]);
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "task_worker_not_recipient")
    );
}
...
#[test]
fn doctor_flags_task_capability_holder_mismatch() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
    ...
    let cap = bus.join("alice-cap.json");
    run(
        &bus,
        &[
            "grant",
    ...
    let mut body: serde_json::Value =
        serde_json::from_str(message["body"].as_str().unwrap()).unwrap();
    body["capability"] = serde_json::from_slice(&fs::read(cap).unwrap()).unwrap();
    message["body"] = serde_json::json!(serde_json::to_string(&body).unwrap());
    fs::write(&path, serde_json::to_vec_pretty(&message).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "task_capability_holder_mismatch")
    );
}
...
#[test]
fn doctor_flags_invalid_receipt_agents() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    receipt["agent"] = serde_json::json!("@bob");
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_receipt_agent_id")
    );
}

#[test]
fn doctor_flags_invalid_receipt_message_ids() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/receipts/{message_id}/bob.json"));
    let mut receipt: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    receipt["message_id"] = serde_json::json!("@message");
    fs::write(&path, serde_json::to_vec_pretty(&receipt).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_receipt_message_id")
    );
}

#[test]
fn doctor_flags_invalid_receipt_conversation_ids() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/receipts/{message_id}/bob.json"));
    let mut receipt: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    receipt["conversation_id"] = serde_json::json!("@c");
    fs::write(&path, serde_json::to_vec_pretty(&receipt).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_receipt_conversation_id")
    );
}

#[test]
fn doctor_flags_invalid_receipt_statuses() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/receipts/{message_id}/bob.json"));
    let mut receipt: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    receipt["status"] = serde_json::json!("finished");
    fs::write(&path, serde_json::to_vec_pretty(&receipt).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "invalid_receipt_status")
    );
}
...
#[test]
fn doctor_flags_empty_receipt_history() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "bob"]);
    run(
    ...
    let path = bus.join(format!("conversations/c/receipts/{message_id}/bob.json"));
    let mut receipt: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
    receipt["history"] = serde_json::json!([]);
    fs::write(&path, serde_json::to_vec_pretty(&receipt).unwrap()).unwrap();
    ...
    assert!(
        report["issues"]
            ...
            .unwrap()
            .iter()
            .any(|issue| issue["code"] == "empty_receipt_history")
    );
}
...
#[test]
fn swarm_assign_rejects_zero_count_with_parse_code() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    run(
        &bus,
    ...
    let denied = run_fail(
        ...
        &[
            ...
            "Nobody",
            "--body",
            "This should fail before selecting workers.",
            "--json",
        ],
    );
    let err: serde_json::Value = serde_json::from_slice(&denied.stderr).unwrap();
...
#[test]
fn channel_create_rejects_unclaimed_members() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    run(&bus, &["claim", "alice", "--workspace", "."]);
...
#[test]
fn task_dispatch_rejects_capability_for_another_holder() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
    ...
    assert!(err["error"]["message"].as_str().unwrap().contains("holder"));
}

#[test]
fn task_dispatch_rejects_capability_for_wrong_tool() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
    ...
    assert!(err["error"]["message"].as_str().unwrap().contains("tool"));
}
...
#[test]
fn task_dispatch_rejects_runtime_beyond_capability() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
...
#[test]
fn task_dispatch_rejects_output_beyond_capability() {
    let bus = temp_bus();
    run(&bus, &["init"]);
    claim_agents(&bus, &["alice", "worker"]);
    run(
    ...
    let err: serde_json::Value = serde_json::from_slice(&denied.stderr).unwrap();
    assert_eq!(err["error"]["code"], "not_authorized");
    assert!(err["error"]["message"].as_str().unwrap().contains("output"));
}
